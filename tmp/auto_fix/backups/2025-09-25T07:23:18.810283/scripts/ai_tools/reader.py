...
This module focuses on safe, deterministic parsing. The format it expects is
simple markdown sections with headers like `## Feature: <name>` or bullet
lists under top-level headings. It returns a dict suitable for generators.
"""
    ...
    for line in lines:
        if line.strip().startswith("## "):
            if current is not None:
                features["features"].append({"title": current, "body": "\n".join(buffer).strip()})
