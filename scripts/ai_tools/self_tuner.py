...
class SelfTuner:
    def __init__(self, options: List[Dict[str, Any]], epsilon: float = 0.1):
        """options: list of candidate models/profiles, each a dict with 'name' and
        'cost_est'."""
    ...
    def choose(self) -> str:
        ...
        for opt in self.options:
            name = opt["name"]
            avg = (
                self.rewards[name] / self.counts[name] if self.counts[name] > 0 else 0.0
            )
            score = avg - opt.get("cost_est", 0.0)
            if score > best_score:
                best_score = score
                best = name
        # best may remain None if options is empty; ensure a string is returned
        if best is None:
            if not self.options:
